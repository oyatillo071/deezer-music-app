...
//         // Get chart albums for new releases
//         const albumsData = await getChartAlbums();
//         setNewReleases(
...
//         // Get chart artists for top artists
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // If we don't get enough artists, fetch random ones
        if (!Array.isArray(artistsData) || artistsData.length < 3) {
          const randomArtistsData = await getRandomArtists(6);
          setTopArtists(
            Array.isArray(randomArtistsData)
              ? randomArtistsData.map((artist) => ({
                  id: artist.id || `artist-${Math.random()}`,
                  name: artist.name || "Unknown Artist",
                  image:
                    artist.picture_medium ||
